...
// test('coapPolka::usage::middleware (async)', async t => {
// 	t.plan(7);
...
// test('coapPolka::usage::middleware (basenames)', async t => {
// 	t.plan(40);
...
// test('coapPolka::usage::middleware (wildcard)', async t => {
// 	t.plan(29);
...
// test('coapPolka::usage::errors', async t => {
// 	t.plan(9);
// 	let a = 41;

// 	// next(Error)
// 	let foo = coapPolka().use((req, res, next) => {
...
// 	// next(string)
// 	let bar = coapPolka().use((_, r, next) => {
// 		next('boo~!');
// 	}).get('/', (_, res) => {
// 		a = 123; // wont run
// 		res.end('OK');
...
// test('coapPolka::usage::sub-apps', async t => {
// 	t.plan(24);
...
// 	let sub = coapPolka().use(bar).get('/', (req, res) => {
...
// 	let app = coapPolka().use(foo).use('sub', sub).get('/', (req, res) => {
...
// test('coapPolka::options::server', async t => {
// 	let server = coap.createServer();
// 	let app = coapPolka({ server });
// 	t.same(app.server, server, '~> store custom server internally as is');

// 	await app.listen();
// 	t.same(server._events.request, app.handler, '~> attach `coapPolka.handler` to custom server');

// 	app.server.close();
// 	t.end();
// });

// test('coapPolka::options::onError', async t => {
// 	t.plan(7);
...
// 	let app = coapPolka({ onError:foo }).use((req, res, next) => next(abc));
...
// test('coapPolka::options::onNoMatch', async t => {
// 	t.plan(6);
...
// 	let app = coapPolka({ onNoMatch:foo }).get('/', _ => {});
